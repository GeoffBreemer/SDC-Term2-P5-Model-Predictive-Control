...
//  size_t i;
  ...
  // Number of model variables, state has 6 elements and 2 actuators
  size_t n_vars =  N * NUM_ELEMENTS + NUM_ACTUATORS * (N - 1);
  size_t n_constraints = N * NUM_ELEMENTS;

  // Initial value of the independent variables, should be 0 except initial state
  Dvector vars(n_vars);
  ...
  // Set all bounds except for the two actuators to max negative and max positive
  for (int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -1.0e19;
    vars_upperbound[i] = 1.0e19;
  }

  // Steering angle actuator bounds
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -1.0;
    vars_upperbound[i] = 1.0;
  }

  // Acceleration actuator bounds
  for (int i = a_start; i < n_vars; i++) {
    vars_lowerbound[i] = -1.0;
    vars_upperbound[i] = 1.0;
  }

  // Constraints should be 0 except initial state
